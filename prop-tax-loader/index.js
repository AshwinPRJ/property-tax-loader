...
BPromise.reduce(data, add2Graph, {}).then(db.close)
...
async function add2Graph(acc, row, index) {
  ...
  let propNode = db.upsertNode('p:Property', ['pid'], propData);
  commandSet.push(propNode);

  // nodes creation...
  let ownerData = getOwnerData(row);
  //let ownerNode = db.upsertNode('person:Person', ['name', 'middle_name'], ownerData);
  let ownerNode = db.upsertNode('person:Person', ['name_KA', 'middle_name_KA'], ownerData);
